...
**宣言的仕様書**とは、「何を達成したいか（What）」を記述した文書であり、「どのように実現するか（How）」の実装詳細を含まない要求記述である。一方、**手続き的マクロ**は、目標達成のための具体的な実行手順を自然言語で記述したものである。
...
宣言的仕様から手続き的マクロへの変換は、要求内容の性質に応じて以下の3つのパターンに分類される。各パターンは、変換の特性と生成される手続きの構造に基づいて体系化されている。
