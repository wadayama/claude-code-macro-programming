...
現実世界の処理は非同期的に発生する。イベント駆動実行は、外部イベントを監視し、変化を検知した時点で自動的にマクロを実行する仕組みである。最も現実的なアプローチは、監視を既存の技術に委ね、LLMはトリガー後の処理に集中するハイブリッド設計である。
...
Pythonコードの品質基準については[A.3: 統合実行のPython品質ガイドライン](#python品質ガイドライン)を参照してください。マルチエージェント環境での協調処理において、型安全性、エラーハンドリング、テスト戦略、再現性の確保が特に重要です。
