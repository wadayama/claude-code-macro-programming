...
**楽観的ロック機能を利用する場合**は、CLAUDE.mdの変数保存ルールを以下のように拡張することで、より安全な並行処理が実現できる：

```markdown
## 拡張された変数保存ルール（楽観的ロック対応）

変数保存指示を受けた場合：

1. **楽観的ロック関数の呼び出し**
   - save_variable(key, value)関数を使用
   - 自動的なバージョン管理と競合回避

2. **成功・失敗の確認**
   - 戻り値が"Success"の場合：保存完了を報告
   - 戻り値がエラーの場合：エラー内容を報告し、必要に応じて再試行
