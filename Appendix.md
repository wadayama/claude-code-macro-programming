...
- [A.7: LLM lint - 総合検証システム](#a7-llm-lint---総合検証システム)
...
自然言語マクロの安全性と実行可能性を以下の5つの観点から体系的に分析する：
...
#### 4. 構文整合性
**検証内容**: 変数参照の整合性、論理的矛盾、無限ループ可能性の分析
**検出例**: 「未定義変数の参照」「ファイル削除後にそのファイル内容を表示」「循環依存」
**判定基準**: 因果関係と時系列の論理的一貫性、変数スコープの妥当性
...
**事前評価との区別**: [A.7 LLM lint検証システム](#a7-llm-lint---総合検証システム)が実行前の静的分析による事前評価を扱うのに対し、本節では実際の出力結果を含む事後評価を対象とします。
...
**評価軸**:
- **論理的整合性**: [A.7: LLM lint検証システム](#a7-llm-lint---総合検証システム)を応用した計画の矛盾や非効率性の静的分析
