...
- [A.7: LLM lint - 実行可能性検証システム](#a7-llm-lint---実行可能性検証システム)
...
## A.7: LLM lint - 実行可能性検証システム

**概要**: 自然言語マクロプログラミングにおいて、AIエージェントによる実際の実行可能性を事前検証するシステム。物理的制約、ツール制限、論理的矛盾、曖昧性を自動検出し、実行前に問題を予防する静的解析ツール。

**4層の防御戦略との関連**: [A.3](#a3-4層の防御戦略)のレイヤー1「設計段階での予防的対策」およびレイヤー4「品質保証テスト」を自動化する中核システムです。

---

### 実行可能性検証の4つの軸

自然言語マクロの実行可能性を以下の4つの観点から体系的に分析する：
...
#### 3. 論理的整合性
**検証内容**: 指示の手順に論理的矛盾がないかの分析
**検出例**: 「ファイル削除後にそのファイル内容を表示」「同時に矛盾する要求」
**判定基準**: 因果関係と時系列の論理的一貫性

#### 4. 具体性・明確性
**検証内容**: 実行に十分な具体性と明確性があるかの評価
**検出例**: 「いい感じにして」「適当にお任せ」「よろしくお願いします」
**判定基準**: 実行可能な具体的指示への変換可能性
...
```bash
# 変数監視の開始
uv run python watch_variables.py --watch

# 出力例（カラー表示）
[14:23:15] 🟢 New: target_macro = "データ分析を実行してください"
[14:23:16] 🟡 Modified: feasibility_result = "FEASIBLE"
[14:23:17] 🟢 New: can_execute = "true"
```

#### 堅牢性設計

- **並行アクセス対応**: SQLite WALモードによる安全な同時実行
- **リトライ機構**: 指数バックオフによる一時的障害の自動回復
- **エラーハンドリング**: 詳細なエラー情報とデバッグ支援

### 実用的価値と差別化

#### examples/self_lintとの差別化

| 機能 | examples/self_lint | llm_lint |
|------|-------------------|----------|
| 検証対象 | セキュリティ・構文チェック | **実行可能性の根本検証** |
| 焦点 | 危険性の検出 | **AI制約を考慮した現実性** |
| 適用範囲 | コード品質 | **自然言語特有の課題** |

#### 他システムとの統合

- **A.3（4層防御戦略）**: レイヤー1の予防的対策として機能
- **A.6（監査ログ）**: 検証履歴の自動記録との連携
- **A.8（メタプログラミング）**: 生成されたマクロの事前検証
- **A.16（SQLite変数管理）**: 共通のデータ基盤による統合

### 実装上の利点

**予防的品質保証**: 実行前の問題検出による時間とリソースの節約
**現実的制約の考慮**: AIエージェントの実際の能力限界を反映した検証
**開発効率の向上**: 曖昧な指示の早期発見と具体化支援
**学習支援**: 自然言語マクロのベストプラクティス習得
...
**事前評価との区別**: [A.7 LLM lint検証システム](#a7-llm-lint---実行可能性検証システム)が実行前の静的分析による事前評価を扱うのに対し、本節では実際の出力結果を含む事後評価を対象とします。
...
**評価軸**:
- **論理的整合性**: [A.7: LLM lint検証システム](#a7-llm-lint---実行可能性検証システム)を応用した計画の矛盾や非効率性の静的分析
