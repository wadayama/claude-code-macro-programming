...
- [A.16: コーディングエージェントによるマクロ自動生成](#a16-コーディングエージェントによるマクロ自動生成)
...
- コマンドライン実行（`claude -p --dangerously-skip-permissions < test_macro.md`）が必須
  - **⚠️ 警告**: `--dangerously-skip-permissions` オプションは通常のセキュリティチェックをバイパスします。このオプションは信頼できる環境でのみ使用し、その危険性を十分理解した上で利用してください。本番環境での使用は推奨されません。
...
## A.16: コーディングエージェントによるマクロ自動生成

### 背景と概念

本節では、宣言的な仕様書から手続き記述を含む自然言語マクロプログラムをClaude Code等のコーディングエージェントで効率的に生成する手法について述べる。

**目標**: 人間が記述する要求仕様（What）を、実行可能な手続き的マクロ（How）に自動変換するプロセスの体系化と、その際の品質向上手法の確立。

#### コーディングエージェントによる高品質マクロ生成

宣言的仕様から手続き的マクロを自動生成する際、コーディングエージェント（Claude Code等）が以下のファイルを事前に読み込むことで、生成品質が大幅に向上する：

- **macro.md**: 10のデザインパターンを共通言語として活用し、構造化された設計パターンに基づくマクロ生成
- **Appendix.md**: 高度なシステム統合手法（マルチエージェント、SQLite連携、エラーハンドリング等）を参照・組み込み

これにより、単純な要求変換ではなく、実用的で保守性の高い構造化マクロコードの生成が可能となる。なお、生成されたマクロの実行には **CLAUDE.md のみ** が必要である。

#### 宣言的仕様書と手続き的マクロの定義

**宣言的仕様書**とは、「何を達成したいか（What）」を記述した文書であり、「どのように実現するか（How）」の実装詳細を含まない要求記述である。宣言的仕様書においては、効率的なマクロ生成のために**デザインパターンによる指示を含める**ことができる（例：「Pattern 2 (Parallel Processing)で複数データソースを処理」）。一方、**手続き的マクロ**は、目標達成のための具体的な実行手順を自然言語で記述したものである。

```markdown
【例】
宣言的仕様: "商品レビューを多角的に分析し、改善提案を生成する"
手続き的マクロ: "Step1: レビューテキストを{{review_text}}に取得
                Step2: 感情分析を実行し{{sentiment}}に保存
                Step3: 要約を生成し{{summary}}に保存
                Step4: 改善提案を{{suggestions}}に生成"
```

既存の[A.7: メタプログラミング](#a7-メタプログラミング)が内部的な動的マクロ生成に焦点を当てるのに対し、本手法は外部エージェントとの協調による生成を特徴とする。また、デザインパターンを共通言語として活用することで、人間とLLM間の意図伝達精度の向上に寄与する。
...
デザインパターンは人間とLLM間で文脈を共有する共通言語として機能する。パターン名の提示により、実装詳細でなく概念レベルでの合意が効率的に形成され、曖昧性の削減による意図齟齬の軽減に寄与する。
...
**迅速なプロトタイピング**: パターン指定により仕様から動作までの時間を短縮

**設計参加の促進**: パターン理解により非プログラマーも設計プロセスに参加可能

**品質向上**: パターンに埋め込まれたベストプラクティスの活用

**経験知の構造化**: ベテランの経験則（「外部API連携は失敗を想定」→Pattern 9 + Pattern 3）をパターン組み合わせで表現

### 適用制約と考慮事項

本手法は以下の場合に適用を慎重に検討する必要がある：

- **単純な一回限りのタスク**: パターン学習コストが効果を上回る場合
- **極めて専門的で独自性の高い要求**: 既存パターンでカバーできない領域
- **リアルタイム性が最重要の処理**: パターン選択による遅延が許容できない場合
- **標準的なプログラミングで十分な場合**: 従来手法の方が効率的な単純タスク

これらの制約を理解した上で、適切な場面での活用により最大の効果を得ることができる。
