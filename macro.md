...
本ガイドは、**自然言語をインタープリタとして構造化されたタスクを実行する**自然言語マクロプログラミングアプローチを提示する。本ガイドではClaude Codeを実行環境として使用する。
...
このフレームワークの核心は、ツールや特定の製品ではなく、**自然言語をインタープリタとして構造化されたタスクを実行する**というアプローチそのものにある。Claude Codeは、このアプローチを実現する優れた実行環境の一つとして位置づけられる。
...
- **高確率動作**: 変数管理（`{{variable_name}}`）、外部モジュール実行（`filename.mdの実行`）等は十分に優れたLLMの場合には非常に高い確率で期待通りに動作する
