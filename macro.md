...
本ガイドは、**Claude Codeを自然言語インタープリタとしてみなして、エージェントプログラミングを実現する**自然言語マクロプログラミングアプローチを提示します。
...
本ガイドで紹介する自然言語マクロプログラミング手法は、LLM（大規模言語モデル）の確率的動作特性に基づいています：

- **高確率動作**: 変数管理（`{{variable_name}}`）、外部モジュール実行（`filename.mdの実行`）等は非常に高い確率で期待通りに動作します
- **完全確定性の非保証**: 100%確定的な動作はLLMの性質上期待できません
- **実用的信頼性**: 実際の使用において十分な信頼性を持つレベルで動作します
- **段階的機能低下**: 理想的動作が困難な場合も、部分的な価値提供を継続します

この特性を理解した上で、実用的な自然言語マクロプログラミングをお楽しみください。
