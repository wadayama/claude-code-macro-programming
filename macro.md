...
自然言語マクロプログラミングは、メタプログラミングとしての側面も持つ。自然言語マクロプログラミングでは、LISPに似た「コードとデータの等価性」を持つ。この特性は、特にメタプログラミング（プログラムを操作するプログラムを書くこと）を容易にする。マクロを動的に生成し、それを改めて自身に組み込み実行させる、といった高度なメタプログラミングが可能である。
