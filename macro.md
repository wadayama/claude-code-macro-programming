...
本ガイドは、**自然言語をインタープリタとして構造化されたタスクを実行する**自然言語マクロプログラミングアプローチを提示する。本ガイドではClaude Codeを実装例として使用する。
...
このフレームワークの核心は、ツールや特定の製品ではなく、**「自然言語をインタープリタとして構造化されたタスクを実行する」**というアプローチそのものにある。Claude Codeは、このアプローチを実現する優れた実装例の一つとして位置づけられる。
...
3. **段階的学習モデルの設計**
   - 基本パターン（順次・並列・条件分岐）から高度パターン（学習・環境理解・人間協調・エラーハンドリング）への体系的進歩
