...
## ⚠️ 重要：事前準備
**実際の自然言語マクロプログラムの実行前に、以下のファイルをClaude Codeに読み込ませることが必須です：**

1. **CLAUDE.md** - 自然言語マクロ構文の定義ファイル（変数管理、条件分岐、モジュール実行等の文法規則）
2. **macro.md**（本ファイル） - 10のデザインパターンと基本概念の解説
3. **Appendix.md** - 高度な技術とシステム統合手法の詳細

これらのファイルを事前に読み込むことで、Claude Codeは適切なコンテキストを把握し、自然言語マクロプログラムを正確に解釈・実行できます。
...
- [A.16: LLMエージェント協調マクロ自動生成](./Appendix.md#a16-llmエージェント協調マクロ自動生成) - 宣言的仕様から手続き的マクロへの自動変換とパターン共通言語
